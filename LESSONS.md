...
## Live transcription during the call does not fit on this CPU, and the thing it risks is the recording

Rolling transcription while recording (chunk the growing WAV, transcribe the deltas, keep a live transcript for mid-meeting questions) was measured against the numbers above and dropped. On the same i7-8550U, `small`/int8 runs at RTF 0.26 per track, but live needs *both* tracks, so the combined figure is ~0.52: it keeps up only on paper. Overlapping windows, the obvious fix for words cut at a chunk boundary, double it (a 30 s window with a 15 s hop lands at RTF ~1.04) and it falls behind and never recovers. Capping `cpu_threads` to leave the videoconference some CPU raises RTF proportionally, straight back over 1.0 — and without a cap faster-whisper takes all 8 threads while the browser is already doing encode, decode and echo cancellation.

The decisive argument is not that the live transcript would lag. It is that saturating a thermally throttled 15 W CPU risks starving `parecord` into xruns, corrupting the capture — the one artifact here that cannot be recovered afterwards. Notes can always be regenerated from a transcript; a dropped recording cannot be regenerated from anything. Post-hoc chat against the finished transcript (`meeting chat`) covers the actual use case at zero risk to the capture, roughly 16 minutes after hanging up for a one-hour meeting. If this is ever revisited on different hardware, the pass/fail criterion is whether the *recording* stayed clean during a real call (RMS plus xrun count), not whether the live transcript looked good.

## A shell hook can't call an MCP tool, but it can call an agent that can

MCP tools exist inside an MCP client, so a `sh -c` hook has no way to invoke one directly; the usual reflex is to give up and write a bespoke API client against whatever the target app exposes, adding a token and a cloud dependency. There is a cheaper path when the machine already has an MCP-capable agent CLI installed: shell out to it in headless mode and let it make the tool calls. `claude -p "<instructions>" --allowedTools "mcp__server__tool_a,mcp__server__tool_b" < payload` inherits the user's existing MCP server configuration, and `--allowedTools` is what makes it usable unattended — headless cannot prompt for permission, so anything not pre-allowed is simply refused rather than blocking. Verified end-to-end from a hook: two MCP write tools named, the payload on stdin, the created object's id echoed back on stdout.

Two things to keep honest about it. The conversion step is a model, so it is not bit-for-bit deterministic: instruct it to move content rather than rewrite it, keep the on-disk file as the source of truth, and never let it guess values for structured fields (an empty field a human fills beats a plausible wrong one). And restrict the tool list to the minimum the job needs — it is the only enforcement boundary in an unattended run.

## A desktop app's local MCP server is only up while the app is: preflight on connection, not HTTP status

An MCP server that ships inside a desktop application (rather than as a standalone daemon) binds its port only while that app runs, so any hook that talks to it must expect it to be absent. Probing it is worth the two seconds: without a preflight, a closed app costs a full model round-trip that then finds no tools and produces a confusing failure instead of a clean skip. Probe by the connection succeeding, not by the HTTP status code — one such endpoint answers `401` to an unauthenticated plain `GET`, which still proves the app is up and listening, whereas a refused connection (`curl` exit 7) is the real "not running" signal. In a best-effort hook the right response to "not running" is to say so and exit 0: a missed push must not look like a failed archive.
